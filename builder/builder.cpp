...
{
public:
  DistrWorkPackage(char const *type)
  {
    sprintf(_desc, "Distributed Work Package for: %s", type);
  }
  void setFile(char const *f, char *v)
  {
    sprintf(_temp, "\n  File(%s): %s", f, v);
    std::strcat(_desc, _temp);
  }
  void setQueue(char const *q, char *v)
  {
    sprintf(_temp, "\n  Queue(%s): %s", q, v);
    std::strcat(_desc, _temp);
  }
  void setPathway(char const *p, char *v)
  {
    sprintf(_temp, "\n  Pathway(%s): %s", p, v);
    std::strcat(_desc, _temp);
  }
